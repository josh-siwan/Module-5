...
// Done ~ TODO - #1: Add support to the 'filter' endpoint for a new query parameter 'letter' which filters friends by starting letter
...
router.get('/info', (req, res) => {
    ...
    for (let header in req.headers) {
        if (header === "accept" || header === "content-type" || header == "user-agent") {
            modifiedHeaders[header] = req.headers[header]
        }}
...
router.get('/:id', (req, res) => {
    ...
    friendFound ? res.status(200).json({result:friendFound}) : res.status(404).json({result:`User ${friendId} not found`}); //FIX THIS UP 
})
